...
function acoes(item) {
  ...
  if (item.status === "aceito" && !item.confirmacoes?.retiradaLoja) return `<button class="btn btn-primary" data-action="retirada">Confirmar coleta e pagamento</button>`;
  ...
  if (item.status === "devolucao" && !item.confirmacoes?.devolucaoLoja) return `<button class="btn btn-primary" data-action="devolucao">Confirmar recebimento da devolução</button>`;
  if (item.status === "devolucao") return `<button class="btn" disabled>Aguardando confirmação do entregador</button>`;
  if (["entregue", "devolvido"].includes(item.status) && item.courierId) return `<button class="btn" data-action="avaliar">Avaliar entregador</button>`;
  return "";
}

function render() {
  const visiveis = entregas.filter(e => filtro === "ativas" ? !["entregue","devolvido","cancelado"].includes(e.status) : ["entregue","devolvido","cancelado"].includes(e.status));
  ...
  document.getElementById("stat-andamento").textContent = entregas.filter(e => ["aceito","retirado","devolucao"].includes(e.status)).length;
  document.getElementById("stat-concluidas").textContent = entregas.filter(e => ["entregue","devolvido"].includes(e.status)).length;
...
async function agir(e) {
  ...
  try {
    if (btn.dataset.action === "valor") {
      ...
      await confirmarEtapa(id, "loja", "retirada", sessao.user.uid); mostrarToast("Coleta e pagamento digital confirmados. A etapa avança quando o entregador também confirmar.", "sucesso");
    ...
    } else if (btn.dataset.action === "devolucao") {
      await confirmarEtapa(id, "loja", "devolucao", sessao.user.uid); mostrarToast("Recebimento da devolução confirmado. O pagamento do entregador foi preservado.", "sucesso");
...
async function init() {
  ...
  onSnapshot(ref, async s => { if (s.exists()) atualizarPerfil(s.data()); else await sair(); }, () => mostrarToast("Não foi possível atualizar o status do cadastro.", "erro"));
  ...
  document.getElementById("form-nova").addEventListener("submit", async e => {
    ...
    const fd = new FormData(e.target); try { await criarEntrega(sessao.user.uid, Object.fromEntries(fd)); e.target.reset(); fecharModal("modal-nova"); mostrarToast("Corrida publicada. O pagamento digital deverá ser confirmado na coleta.", "sucesso"); } catch (err) { mostrarToast(err.message === "PERFIL_INATIVO" ? motivoBloqueio(loja) : err.message === "FORMA_PAGAMENTO_INVALIDA" ? "Escolha Pix ou pagamento online para pagar o entregador na coleta." : mensagemErroAmigavel(err), "erro"); } finally { btn.disabled = false; }
  });
