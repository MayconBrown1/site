...
import { exigirPerfil, sair } from "./permissions.js";
import { doc, getDoc, onSnapshot, updateDoc } from "https://www.gstatic.com/firebasejs/12.18.0/firebase-firestore.js";
import { aceitarEntrega, confirmarEtapa, iniciarDevolucao, motivoBloqueio, observarEntregasDisponiveis, observarEntregasDoEntregador } from "./pedidos.js";
...
let sessao, perfil, disponiveis = [], minhas = [], filtro = "disponiveis", entregaAvaliacao = null, entregaDevolucao = null, nota = 0;
...
function acoes(item, disponivel = false) {
  ...
  if (item.status === "aceito" && !item.confirmacoes?.retiradaEntregador) return `<button class="btn btn-primary" data-action="retirada">Confirmar coleta e recebimento</button>`;
  if (item.status === "retirado" && !item.confirmacoes?.entregaEntregador) {
    const entrega = item.confirmacoes?.entregaLoja ? `<button class="btn btn-primary" data-action="entrega">Confirmar entrega</button>` : `<button class="btn" disabled>Aguardando confirmação do estabelecimento</button>`;
    return `${entrega}<button class="btn btn-danger" data-action="iniciar-devolucao">Cliente não recebeu</button>`;
  }
  if (item.status === "devolucao" && !item.confirmacoes?.devolucaoEntregador && item.confirmacoes?.devolucaoLoja) return `<button class="btn btn-primary" data-action="devolucao">Confirmar devolução ao estabelecimento</button>`;
  if (item.status === "devolucao" && !item.confirmacoes?.devolucaoLoja) return `<button class="btn" disabled>Aguardando o estabelecimento receber a devolução</button>`;
  if (item.status === "devolucao") return `<button class="btn" disabled>Aguardando o estabelecimento confirmar a devolução</button>`;
  if (["entregue", "devolvido"].includes(item.status)) return `<button class="btn" data-action="avaliar">Avaliar estabelecimento</button>`;
  return "";
}
...
function render() {
  ...
  if (filtro === "andamento") { itens = minhas.filter(e => ["aceito","retirado","devolucao"].includes(e.status)); vazio = ["Nenhuma entrega ativa", "Aceite uma corrida disponível para começar."]; }
  if (filtro === "historico") { itens = minhas.filter(e => ["entregue","devolvido","cancelado"].includes(e.status)); vazio = ["Histórico vazio", "Suas entregas concluídas aparecerão aqui."]; }
...
async function agir(e) {
  ...
  try {
    ...
    else if (btn.dataset.action === "retirada") { await confirmarEtapa(id, "entregador", "retirada", sessao.user.uid); mostrarToast("Coleta e recebimento digital confirmados.", "sucesso"); }
    ...
    else if (btn.dataset.action === "iniciar-devolucao") { entregaDevolucao = item; document.getElementById("form-devolucao").reset(); abrirModal("modal-devolucao"); }
    else if (btn.dataset.action === "devolucao") { await confirmarEtapa(id, "entregador", "devolucao", sessao.user.uid); mostrarToast("Devolução confirmada. Seu pagamento continua preservado.", "sucesso"); }
...
async function init() {
  ...
  atualizarAcesso();
  onSnapshot(ref, async s => { if (s.exists()) { perfil = s.data(); document.getElementById("switch-online").classList.toggle("on", !!perfil.online); document.getElementById("texto-online").textContent = perfil.online ? "Online" : "Offline"; atualizarAcesso(); render(); } else await sair(); });
  ...
  document.getElementById("form-devolucao").addEventListener("submit", async e => {
    ...
    try {
      ...
      await iniciarDevolucao(entregaDevolucao.id, sessao.user.uid, fd.get("motivo"), fd.get("observacoes"));
      fecharModal("modal-devolucao");
      mostrarToast("Devolução iniciada. Retorne o produto ao estabelecimento; seu pagamento permanece garantido.", "sucesso");
    } catch (err) { mostrarToast(mensagemErroAmigavel(err), "erro"); }
    finally { btn.disabled = false; }
  });
